...
// Known tool IDs that should be assigned
...
async function fixPromptWithTools() {
  console.log('\n🔧 FIXING PROMPT + RESTORING TOOLS\n');
...
You will receive these variables as template values:
- {{firstName}} - Customer's first name
- {{appointmentTime}} - Full appointment time with date
- {{appointmentTimeOnly}} - Just the time portion
...
- Understanding and empathetic`;
    ...
    console.log('📝 Updating assistant with prompt + tools...\n');
    ...
    await axios.patch(
      `https://api.vapi.ai/assistant/${CONFIRMATION_ASSISTANT_ID}`,
      {
        model: {
          ...
          toolIds: REQUIRED_TOOL_IDS  // ← CRITICAL: Re-add tool IDs
        }
      },
      {
        headers: {
    ...
    console.log('   ✅ Prompt updated with {{firstName}} and {{appointmentTimeOnly}}');
    console.log('   ✅ Restored all 4 tools:');
    ...
    console.log('\n   🎯 Assistant is now fully configured!\n');
